...
async function fetchKpIndex(): Promise<{ kp: number; timestamp: string }> {
  try {
    ...
    // Handle both object format {time_tag, Kp} and array format [time_tag, Kp, ...]
    const kp = typeof latest === 'object' && !Array.isArray(latest)
      ? (latest.Kp ?? 0)
      : parseFloat(latest[1]) || 0
    const timestamp = typeof latest === 'object' && !Array.isArray(latest)
      ? (latest.time_tag || new Date().toISOString())
      : (latest[0] || new Date().toISOString())
    return { kp, timestamp }
  } catch (err) {
...
async function fetchSolarWind(): Promise<{
  ...
  try {
    ...
    // Walk backwards to find the latest row with valid values
...
async function fetchIMF(): Promise<{ bz: number; bt: number }> {
  try {
    ...
    console.error('Error fetching IMF:', err)
    return { bz: 0, bt: 0 }
  }
}
...
export async function fetchSpaceWeather(): Promise<SpaceWeatherResult> {
  ...
  const [kpData, solarWind, imf] = await Promise.all([
    fetchKpIndex(),
    fetchSolarWind(),
    fetchIMF(),
  ])
